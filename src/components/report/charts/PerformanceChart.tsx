...
const PerformanceChart = ({ report }: any) => {
    const t = useTranslations('Report');

    const data = [
        { name: 'Score', value: Number((report.performance.performanceScore * 100).toFixed(0)) - 1 },
        { name: 'Remainder', value: 100 - Number((report.performance.performanceScore * 100).toFixed(0)) + 1 },
    ];
        ...
        <div className='flex flex-col items-center gap-2'>
            ...
            <div className="relative w-fit">
                ...
                <p className="absolute top-[50%] translate-y-[-50%] left-[50%] translate-x-[-50%] text-xl">
                    {(report.performance.performanceScore * 100 - 1).toFixed(0) + '%'}
                </p>
            </div>
        </div>
    )
